...
namespace LazyShortcuts
{
    ...
    {
        public static bool enableResetTransform = true,
            enableStats = true,
            enableMutePlay = true,
            enableResetName = true,
            enableMaxPlay = true,
            enableRevertPrefab = true,
            enableSaveChangesToPrefab = true,
            enableToggleDebugNormalView = true,
            enableToggleLockUnlockInspector = true;
        ...
        static void ToggleMaxPlay()
        {
            ...
            foreach (EditorWindow editorWindow in editorWindows)
            {
                if (editorWindow.GetType().Name == "GameView")
                {
                    ...
                    PropertyInfo propertyInfo = gameViewWindow.GetType()
                        .GetProperty("maximizeOnPlay", BindingFlags.Public | BindingFlags.Instance);
        ...
        static void ToggleStats()
        {
            ...
            foreach (EditorWindow editorWindow in editorWindows)
            {
                ...
                {
                    FieldInfo fieldInfo = gameViewWindow.GetType()
                        .GetField("m_Stats", BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            foreach (EditorWindow editorWindow in editorWindows)
            {
                if (editorWindow.GetType().Name == "InspectorWindow")
                {
                    if (EditorWindow.focusedWindow == editorWindow)
                    {
                        ...
                        MethodInfo methodInfo = inspectorWindow.GetType()
                            .GetMethod("SetMode", BindingFlags.NonPublic | BindingFlags.Instance);
                        FieldInfo field = inspectorWindow.GetType()
                            .GetField("m_InspectorMode", BindingFlags.NonPublic | BindingFlags.Instance);
